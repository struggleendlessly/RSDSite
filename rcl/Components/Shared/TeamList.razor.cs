...
namespace rcl.Components.Shared
{
    public partial class TeamList : IDisposable
    {
        ...
        protected override async Task OnInitializedAsync()
        {
            ...
            var serviceItemsKey = string.Format(StaticStrings.ContactUsPageTeamListDataJsonMemoryCacheKey, SiteNameLower);
            if (!MemoryCache.TryGetValue(serviceItemsKey, out List<ServiceItem> serviceItems))
            {
                var jsonContent = await BlobStorageManager.DownloadFile(SiteNameLower, StaticStrings.AboutUsPageTeamListDataJsonFilePath);
        ...
        public async Task SaveContent(PageModel model)
        {
            ...
            await BlobStorageManager.UploadFile(SiteNameLower, StaticStrings.AboutUsPageTeamListDataJsonFilePath, stream);
